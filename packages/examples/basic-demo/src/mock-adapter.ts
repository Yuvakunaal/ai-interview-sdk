...
/**
 * A fake AIProviderAdapter for the demo — no API key, no network call.
 * It parses the structured prompt @interview-sdk/core sends (see
 * packages/core/src/evaluation/prompt.ts and follow-up/prompt.ts) well
 * enough to return plausible scores and a follow-up question, purely so
 * the widget has something to render. Never use this in a real app —
 * plug in one of the @interview-sdk/adapter-* packages instead.
 */
