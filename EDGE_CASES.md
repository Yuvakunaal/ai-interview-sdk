...
| Version mismatches / breaking-change handling | ⚠️     | Changesets tooling has been exercised for real across multiple releases (e.g. `@interview-sdk/core` `0.0.0` → `0.2.0`, `@interview-sdk/react` → `0.2.3`) with independent per-package versioning working as expected. What's still untested: an actual major (breaking) semver bump — every release so far has been a 0.x minor/patch. |
| Backward compatibility                        | N/A    | All packages are pre-1.0 (`0.x`), so semver's backward-compatibility guarantee doesn't formally apply yet — expect occasional breaking changes between minor versions until 1.0. |
